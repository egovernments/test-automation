...
Background:
	...
	* def searchLicenseRequest = read('../../municipal-services/requestPayload/trade-license/searchLicense.json')
...
@searchTLLicenseSuccessfully
#R2.6 Search License API
Scenario: Search a TL License
	Given url searchTradeLicense
	And params searchLicenseParams
	And request searchLicenseRequest
	When method post
	Then status 200
	And def tradeLicenseResponseHeaders = responseHeaders
	And def tradeLicenseResponseBody = response
  And def tradeLicense = tradeLicenseResponseBody.Licenses[0]
  And def tradeLicenseNumber = tradeLicense.connectionNo
  And def tlApplicationNumber = tradeLicense.applicationNumber