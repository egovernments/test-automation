Feature: BPA-Service pretests
    Background:
        ...
        * configure ssl = false
        * def searchBPAREGRequest = read('../../municipal-services/requestPayload/bpa-services/bparegsearch.json')
   ...
   @approveFromCitizen
    ...
    @searchBPAREGSuccessfully
    Scenario: Search BPAREG Successfully

        Given url searchbparegURL
        And params getBPAREGSearchParam
        And request searchBPAREGRequest
        When method post
        Then status 200
        And def bparegResponseHeaders = responseHeaders
        And def bparegResponseBody = response
        # * print bpaResponseBody